...
class LoginPhase():
    ...
    def setWelcomeWindow(self):
        ...
        self.welcomWindow.addstr(3,20,"use TAB to change input box, ENTER to submit")
    ...
    def task(self):
        while not self.isLogin:
            ...
            self.checkNetwork()
        return communicateWindow(self.loginID)
...
class LoginWindow():
    def __init__(self,n_row, n_col, start_y, start_x) -> None:
        self.content = {"mode":"login", "id" : "", "username": "","password" : ""}
    ...
    def update(self):
        ...
        if self.inputBox.cursor == "password":
...
class debugWindow():
    ...
    def printPassword(self, something):
        self.window.erase()
        if (something != -1):
            self.window.addstr('*' * len(something))
        self.window.refresh()